...
int CommRecv(Domain& domain, int msgType, Index_t xferFields,
   ...
   domain.recvRequest = (MPI_Request *) calloc(26, sizeof(MPI_Request));
   ...
   if (planeMin && doRecv) {
      ...
      MPIX_SAFE_CALL(MPI_Irecv(&domain.commDataRecv[pmsg * maxPlaneComm],
                recvCount, baseType, fromRank, msgType,
                MPI_COMM_WORLD, &domain.recvRequest[pmsg]), code = MPIX_TRY_RELOAD, fail_return) ;
      ++pmsg ;
   }
   if (planeMax) {
      /* contiguous memory */
   ...
   if (!planeOnly) {
      ...
      if (rowMin && colMin && doRecv) {
         ...
         MPIX_SAFE_CALL(MPI_Irecv(&domain.commDataRecv[pmsg * maxPlaneComm +
                   ...
                   MPI_COMM_WORLD, &domain.recvRequest[pmsg+emsg]), code = MPIX_TRY_RELOAD, fail_return) ;
         ++emsg ;
      }
      ...
      if (rowMin && colMin && planeMin && doRecv) {
         ...
         MPIX_SAFE_CALL(MPI_Irecv(&domain.commDataRecv[pmsg * maxPlaneComm +
                   ...
                   MPI_COMM_WORLD, &domain.recvRequest[pmsg+emsg+cmsg]), code = MPIX_TRY_RELOAD, fail_return) ;
         ++cmsg ;
      }
...
int CommSend(Domain& domain, int msgType,
              ...
              Index_t dx, Index_t dy, Index_t dz, bool doSend, bool planeOnly)
{
   ...
   domain.sendRequest = (MPI_Request *) calloc(26, sizeof(MPI_Request));
   ...
   if (planeMin | planeMax) {
      ...
      if (planeMin) {
         ...
         MPIX_SAFE_CALL(MPI_Isend(destAddr, xferFields*sendCount, baseType,
                   myRank - domain.tp()*domain.tp(), msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg]), code = MPIX_TRY_RELOAD, fail_return) ;
         ++pmsg ;
      }
      if (planeMax && doSend) {
         destAddr = &domain.commDataSend[pmsg * maxPlaneComm] ;
   ...
   if (!planeOnly) {
      if (rowMin && colMin) {
         ...
         MPIX_SAFE_CALL(MPI_Isend(destAddr, xferFields*dz, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg]), code = MPIX_TRY_RELOAD, fail_return) ;
         ++emsg ;
      }
      ...
      if (rowMin && planeMin) {
         ...
         MPIX_SAFE_CALL(MPI_Isend(destAddr, xferFields*dx, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg]), code = MPIX_TRY_RELOAD, fail_return) ;
         ++emsg ;
      }
      ...
      if (colMin && planeMin) {
         ...
         MPIX_SAFE_CALL(MPI_Isend(destAddr, xferFields*dy, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg]), code = MPIX_TRY_RELOAD, fail_return) ;
         ++emsg ;
      }
      ...
      if (rowMin && colMin && planeMin) {
         ...
         MPIX_SAFE_CALL(MPI_Isend(comBuf, xferFields, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg+cmsg]), code = MPIX_TRY_RELOAD, fail_return) ;
         ++cmsg ;
      }
   ...
   MPIX_SAFE_CALL(MPI_Waitall(26, domain.sendRequest, status), code = MPIX_TRY_RELOAD, fail_return) ;
   return 0;
   fail_return:
    return MPIX_TRY_RELOAD;
}
...
int CommSBN(Domain& domain, int xferFields, Domain_member *fieldData) {
   int code = MPI_SUCCESS;
   if (domain.numRanks() == 1)
      return 0;
   ...
   if (planeMin | planeMax) {
      ...
      if (planeMin) {
         ...
         MPIX_SAFE_CALL(MPI_Wait(&domain.recvRequest[pmsg], &status), code = MPIX_TRY_RELOAD, fail_return) ;
         for (Index_t fi=0 ; fi<xferFields; ++fi) {
   ...
   if (rowMin & colMin) {
      srcAddr = &domain.commDataRecv[pmsg * maxPlaneComm +
                                       emsg * maxEdgeComm] ;
      MPIX_SAFE_CALL(MPI_Wait(&domain.recvRequest[pmsg+emsg], &status), code = MPIX_TRY_RELOAD, fail_return) ;
      for (Index_t fi=0 ; fi<xferFields; ++fi) {
         Domain_member dest = fieldData[fi] ;
         for (Index_t i=0; i<dz; ++i) {
   ...
   if (rowMin & colMin & planeMin) {
      ...
      Real_t *comBuf = &domain.commDataRecv[pmsg * maxPlaneComm +
                                             emsg * maxEdgeComm +
                                      cmsg * CACHE_COHERENCE_PAD_REAL] ;
      MPIX_SAFE_CALL(MPI_Wait(&domain.recvRequest[pmsg+emsg+cmsg], &status), code = MPIX_TRY_RELOAD, fail_return) ;
      for (Index_t fi=0; fi<xferFields; ++fi) {
...
int CommSyncPosVel(Domain& domain) {
   int code = MPI_SUCCESS;
   if (domain.numRanks() == 1)
      return 0;
