...
void write(int epoch, int rank, Domain& locDom, struct cmdLineOpts &opts) {
   std::ofstream t;
   char buf[50];
   sprintf(buf, "/p/lscratchh/sultana1/check_%d_%d", epoch, rank);
  ...
  // printf("In file write...\n");
   ...
   int size;
	...
	/*t.write(reinterpret_cast<char *>(&locDom.m_e_cut), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_p_cut), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_q_cut), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_v_cut), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_u_cut), sizeof(Real_t));

	t.write(reinterpret_cast<char *>(&locDom.m_hgcoef), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_ss4o3), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_qstop), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_monoq_max_slope), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_monoq_limiter_mult), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_qlc_monoq), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_qqc_monoq), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_qqc), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_eosvmax), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_eosvmin), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_pmin), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_emin), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_dvovmax), sizeof(Real_t));
	t.write(reinterpret_cast<char *>(&locDom.m_refdens), sizeof(Real_t));*/
...
//   printf("In file Read...\n");
   int sz;
	...
	Real_t var;
	/*t.read(reinterpret_cast<char *>(&var), sizeof(int));
	locDom.m_e_cut = var;
	...
	t.read(reinterpret_cast<char *>(&var), sizeof(int));
	locDom.m_monoq_max_slope = var;

	t.read(reinterpret_cast<char *>(&var), sizeof(int));
	locDom.m_monoq_limiter_mult = var;

	t.read(reinterpret_cast<char *>(&var), sizeof(int));
	locDom.m_qlc_monoq = var;

	t.read(reinterpret_cast<char *>(&var), sizeof(int));
	locDom.m_qqc_monoq = var;

	t.read(reinterpret_cast<char *>(&var), sizeof(int));
	locDom.m_qqc = var;

	t.read(reinterpret_cast<char *>(&var), sizeof(int));
	locDom.m_eosvmax = var;

	t.read(reinterpret_cast<char *>(&var), sizeof(int));
	locDom.m_eosvmin = var;
	...
	t.read(reinterpret_cast<char *>(&var), sizeof(int));
	locDom.m_dvovmax = var;

	t.read(reinterpret_cast<char *>(&var), sizeof(int));
	locDom.m_refdens = var;*/
