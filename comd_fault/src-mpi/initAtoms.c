...
/// Set momenta to zero.
int createFccLattice(int nx, int ny, int nz, real_t lat, SimFlat* s)
{
	int rc = 0;
   ...
   rc = addIntParallel(&s->atoms->nLocal, &s->atoms->nGlobal, 1);
   if (rc != 0) {
	   return 101;
   }
...
void writeAtoms(FILE *fp, SimFlat* sim) {
	int local;
	local = sim->atoms->nLocal;
	fwrite(&local, sizeof(int), 1, fp);

	int global;
	global = sim->atoms->nGlobal;
	fwrite(&global, sizeof(int), 1, fp);
