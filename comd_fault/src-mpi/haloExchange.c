...
static int exchangeData(HaloExchange* haloExchange, void* data, int iAxis);
...
int haloExchange(HaloExchange* haloExchange, void* data)
{
	int rc = 0;
   for (int iAxis=0; iAxis<3; ++iAxis) {
      rc = exchangeData(haloExchange, data, iAxis);
      if (rc != 0) {
    	  return 101;
      }
   }
...
///                       unload functions
int exchangeData(HaloExchange* haloExchange, void* data, int iAxis)
{
...
void writeForceHaloExchange(FILE *fp, HaloExchange* hh) {
	...
	int buf_capacity = hh->bufCapacity;
	fwrite(&buf_capacity, sizeof(int), 1, fp);
	//parms--> ForceExchangeparms
...
void writeAtomHaloExchange(FILE *fp, HaloExchange* hh) {
	...
	//parms-->AtomExchangeParms
	AtomExchangeParms* p = (AtomExchange*) hh->parms;
	fwrite(p->nCells, sizeof(int), 6, fp);
	for (int i = 0; i < 6; i++) {
