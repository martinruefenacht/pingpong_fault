...
/************************/
SimFlat* application_checkpoint_read(int rank, int *iStep, int *isEam, Validate** validate) {
	...
	sim->nSteps = steps;
	int prate;
	fread(&prate, sizeof(int), 1, fp);
	sim->printRate = prate;

	double tstep;
	fread(&tstep, sizeof(double), 1, fp);
	sim->dt = tstep;
	...
	/******** BasePotential ********/
	fread(isEam, sizeof(int), 1, fp);
	if(*isEam) {
		sim->pot = readEamPot(fp, sim->boxes);
	}
	else {
...
void application_checkpoint_write(int rank, int iStep, SimFlat* sim, int isEam, Validate* validate) {
	FILE *fp;
	char filename[20];
	sprintf(filename, "check_%d", rank);
	...
	int prate;
	prate = sim->printRate;
	fwrite(&prate, sizeof(int), 1, fp);

	double timestep;
	timestep = sim->dt;
	fwrite(&timestep, sizeof(double), 1, fp);

	real_t ep = sim->ePotential;
	real_t ek = sim->eKinetic;
	fwrite(&ep, sizeof(real_t), 1, fp);
	fwrite(&ek, sizeof(real_t), 1, fp);

	/********** Domain *********/
	writeDomain(fp, sim->domain);
	...
	int atomicno;
	atomicno = sim->species->atomicNo;
	fwrite(&atomicno, sizeof(int), 1, fp);

	real_t m = sim->species->mass;
	fwrite(&m, sizeof(real_t), 1, fp);

	/************* BasePotential ************/
	fwrite(&isEam, sizeof(int), 1, fp);
	if(isEam) {
		writeEamPot(fp, sim->pot, sim->boxes);
	}
	else {
...
int main_loop(int argc, char **argv, int epoch, int *done) {
	...
	if( epoch > 0 ) {
		initSubsystems();
		sim = application_checkpoint_read(getMyRank(), epoch, &iStep, &isEam, &validate);
		timestampBarrier("Restarting simulation\n");
		printf("iStep and nStep %d %d\n", iStep, sim->nSteps);
		nSteps = sim->nSteps;
	...
	for (; iStep<nSteps;)
	{
		...
		iStep += printRate;
		code = timestampBarrier("Iteration end\n");
		if (code != 0) {
			return 101;
		}
		application_checkpoint_write(getMyRank(), epoch, iStep, sim, isEam, validate);
...
{
#if REDIRECT_OUTPUT
	freopen("testOut.txt","a",screenOut);
#endif
...
int sumAtoms(SimFlat* s)
{
	...
	startTimer(commReduceTimer);
	int rc = addIntParallel(&s->atoms->nLocal, &s->atoms->nGlobal, 1);
	if (rc != MPI_SUCCESS) {
		   return MPIX_TRY_RELOAD;
	   }
	stopTimer(commReduceTimer);
